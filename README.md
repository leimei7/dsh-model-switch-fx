...
```
0.20–1.45  线稿 · 匕首      柄 → 刀身
1.45–1.62  停顿             让「刀」这个状态被看清
1.62–2.70  线稿 · 玫瑰
2.70–2.86  停顿
2.86–3.30  线稿 · 藤茎
3.30–3.75  气口             线稿完整，整体静止 450ms
3.75–4.75  磷光加热         颜色从花心向外烧过去
4.75–5.30  落定
```

两条硬规则：

- **线稿必须 100% 画完、再静止 450ms，之后才上色。** 实现上是一条半径为 0 的遮罩把上色层完全封住，直到那一刻才解封 —— 若用「整段淡入」，填充必然和线稿重叠，两幕就糊在一起了
...
- 素材只用**形状**（[game-icons](https://game-icons.net) 的 `dagger-rose`，CC BY 3.0）。它原本是**实心剪影**，这里把它转成线稿再重新上色
- **一个光源管全图**：所有描边与填充都引用一个以花心为圆心、`userSpaceOnUse` 的径向渐变。渐变落在画面坐标上，所以每一条线**自己就带着光的衰减**（靠花心亮而暖、越远越深越冷），而不是「每个部件刷一个平色」
- **45° 明暗 + 层间投影**：光从左上、右下暗；匕首投一层影在花上（影被遮罩限制在「花与藤」的形状内，不会洒进黑场）
- **冷极**：底色偏冷，另有一束 Teal 环境光 —— 全是暖色会失去温差，画面会发闷
