...
```
0.10–1.20  线稿 · A          刀（柄 → 刀身）→ 玫瑰 → 藤
1.24–2.10  线稿 · B          五片依次描出 + 两条轴
1.98–2.24  ★ X 落下          两笔交叉 —— 两件东西成为一件
2.20–2.40  气口              整体静止
2.38–3.10  加热              A 上色 / B 变亮（两个遮罩各从自己的光源向外解封）
2.45–3.10  字标               K I K I 逐格锁定
3.10–3.85  ★ 定格保持         什么都不发生的这一段才是「贵」的地方
```

**落点是那个 X。** A 画完、B 画完，最后 X 才落下把它连成一件 —— 没有它，这只是两个图形并排放着；有了它，才是一次联名。

三条硬规则：

- **线稿必须 100% 画完 + 气口之后才上色。** 实现上是一条半径为 0 的遮罩把上色层完全封住，直到那一刻才解封 —— 若用「整段淡入」，填充必然和线稿重叠，两幕就糊在一起了
- **A 与 B 一个维度上对立、另一个维度上押韵。** 对立在色彩（A 金+玫红 / B 只有黑白灰），押韵在同一套处理（磷光、辉光、扫描线、颗粒）+ 同一支笔（线稿 → 气口 → 加热 → 定格）。合起来的框架是「**一台彩色显像管 + 一台黑白显像管**」并排。B 用黑白灰正是因为 A 已经有颜色了
- **一次性。** 已经是开的时候再打 `<kiki:on>`、已经是关的时候再打 `<kiki:off>`，状态没变，不重播

**关闭是把同一条时间轴倒放** —— 开机是画出来、再上色；关机是褪色、再拆掉。
