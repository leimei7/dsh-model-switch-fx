...
var SFX_GAIN = {"ready":0.642,"disk":0.861,"ensemble":0.296,"call":0.39,"arch":0.728,"stab":0.762,"horn":0.362,"glide":0.518,"zigzag":0.629,"stepwise":0.566,"single":1.069,"poise":0.671}
...
/* SFX-END */
  ...
  function sfxSchedule(ctx, dest, plan, root, gain) {
    ...
    for (var vi = 0; vi < plan.voices.length; vi++) {
      ...
      for (var ei = 0; ei < v.events.length; ei++) {
        ...
        g.gain.linearRampToValueAtTime(vg, at + SFX_ATTACK_S)
        g.gain.exponentialRampToValueAtTime(
          Math.max(vg * 0.22, 1e-4), at + Math.min(len, SFX_ATTACK_S + SFX_DECAY_S))
  ...
  /** 播一段角色动画 + 语音 + 启动音。 */
  function show(id, voice, volume, token, sfxOn, sfxVolume) {
    ...
    try {
      ...
      try {
        ...
        audio.preload = 'auto'
        audio.volume = typeof volume === 'number' ? volume : 0.9
        entry.audio = audio
        entry.timers.push(window.setTimeout(function () {
          if (active !== entry) return
    ...
    try {
      ...
      es.onmessage = function (event) {
        ...
        if (msg && msg.type === 'play') {
          show(msg.id, msg.voice, msg.volume, msg.token, msg.sfx, msg.sfxVolume)
        }
      }
