...
(function () {
  ...
  /** 播一段角色动画 + 语音。 */
  function show(id, voice, volume, token) {
    ...
    try {
      ...
      // 正常模式**不立刻播**，等 logo 先动起来再出声；减少动效模式没有描边要等，
      // 用更短的延迟。两种情况都是先建好元素，延迟期间正好用来预加载。
      var audioDelay = reduced ? REDUCED_AUDIO_DELAY_MS : AUDIO_DELAY_MS
      ...
      var total = reduced
        ? Math.max(REDUCED_AUDIO_DELAY_MS + voiceMs + TAIL_MS, NAME_HOLD_MS + FADE_MS)
        : Math.max(audioDelay + voiceMs + TAIL_MS, drawEnd + NAME_HOLD_MS + FADE_MS)
  ...
  function connect() {
    try {
      ...
      es.onmessage = function (event) {
        ...
        if (msg && msg.type === 'play') show(msg.id, msg.voice, msg.volume, msg.token)
      }
