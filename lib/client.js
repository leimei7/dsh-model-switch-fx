...
(function () {
  ...
  function show(id, voice, volume, token) {
    ...
    try {
      ...
      // 现在改成显式三步：先钉死隐藏态并提交，再布动画，最后才显示浮层。
      ...
      for (var i = 0; i < paths.length; i++) {
        ...
        paths[i].style.strokeDashoffset = String(DASH_LEN)
      }
      ...
      if (solidGroup) {
        ...
        // 否则实心层永远不亮（最终染色动画就没了）。
        solidGroup.style.opacity = '0'
        entry.solid = solidGroup
      }

      // 4) 提交这个隐藏态。此刻浮层还没有 dsf-on（opacity:0），看不见。
      ...
      // 6) 动画已经就位，这才让浮层可见并接管输入
      root.classList.add('dsf-on')
      lockInput()

      // 语音：触发点是用户点选模型，页面已有交互手势，autoplay 允许。
      // 现在**不立刻播**，放到 AUDIO_DELAY_MS 之后 —— 让 logo 先动起来再出声，
      // 开场不至于一上来就有人在说话。元素先建好，延迟期间正好用来预加载。
      ...
      var total = Math.max(
        AUDIO_DELAY_MS + voiceMs + TAIL_MS,
